...
def plot_model_corr(cvR2, regressor, c_max = 0.25, localdisk = None):
    '''
    Plots cross-validated R^2 results
    cvR2            : Cross-validated R^2 array
    regressor       : Name of the regressor or regressor category used in constructing the model
    ...
    if not localdisk is None:
        ...
        if not os.path.isdir(folder):
            os.makedirs(folder)
        fig.savefig(pjoin(folder,f'{regressor}_model_corr.pdf'))